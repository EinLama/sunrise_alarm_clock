/*
   Sunrise project
   02/27/2018 Tobias Eilert

   Based mostly on https://github.com/bruhautomation/ESP-MQTT-JSON-Digital-LEDs
   Thank you, @bruh!

   Known settings for my nodemcu board:
   NodeMCU 1.0 (ESP-12E Module), 160MHz, 4M (1M SPIFFS), v2 Lower Memory, Serial, None, Only Sketch


   Cool effect: cyclon rainbow -> unfortunately broken as of now. Target: fix it!
   Ideas of how to do this:
 * * use FastLED.delay instead of delay
...
// Interrupt problems, see https://github.com/FastLED/FastLED/wiki/Interrupt-problems
// The length of our WS2811 LED strip introduces timing problems with our ESP8266. The strip requires tight timing, which FastLED takes care of.
// However, we can't do anything else when FastLED is writing to our strip -> when FastLED is too busy, it disables the rest of our logic.
// We can try to set these values before including FastLED:
...
/********************************** START SETUP*****************************************/
void setup() {
  //Serial.begin(115200); // bruh version
  Serial.begin(9600); // adjusted! Wifi connection works when this number is set, see https://github.com/esp8266/Arduino/issues/2186#issuecomment-228581052
...
/********************************** START SETUP WIFI*****************************************/
void setup_wifi() {
  //Serial.setDebugOutput(true); // uncomment this to debug wifi connection
...
void loop() {
  ...
  if (effectString == "cyclon rainbow") {                    //Single Dot Down
    if (transitionTime == 0 or transitionTime == NULL or transitionTime < 20) {
      transitionTime = 20; // slower than this and the OS will crash and not respond anymore -> too intense!
    }
...
void showleds() {
  ...
  if (stateOn) {
    ...
    //FastLED.show(); // this was here previously. Moved to the else branch of below if condition since FastLED.delay already calls FastLED.show

    if (transitionTime >= 1 && transitionTime < 200) {  //Sets animation speed based on receieved value
      FastLED.delay(1000 / transitionTime); // FastLED.delay calls FastLED.show, so we don't need to call it seperately.
